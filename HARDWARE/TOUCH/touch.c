...
//获取方向
u8 ret;
float max_cos, cos_up, cos_down, cos_left, cos_right, vec_len;
u8 getDir(TouchPoint pathB, TouchPoint pathE, u8 mode)
{
//for debug
	extern u16 offsetX;
	extern u16 offsetY;

//	gui_fill_circle(pathB.x, pathB.y,20,BLUE);
//	gui_fill_circle(pathE.x, pathE.y,20,RED);
	ret = 10;
	max_cos = -1000000.0;
//vec_len = sqrt((pathE.x-pathB.x)^2+(pathE.y-pathB.y)^2);	
	vec_len = sqrt((pathE.x-pathB.x)*(pathE.x-pathB.x)+(pathE.y-pathB.y)*(pathE.y-pathB.y));
	cos_up = -(pathE.y-pathB.y) / vec_len;
	if(cos_up>=max_cos) {ret = UP; max_cos=cos_up;}
	
	cos_down = -cos_up;
	if(cos_down>=max_cos) {ret = DOWN; max_cos=cos_down;}
	
	cos_left = -(pathE.x-pathB.x) / vec_len;
	if(cos_left>=max_cos) {ret = LEFT; max_cos=cos_left;}
	
	cos_right = -cos_left;
	if(cos_right>=max_cos) {ret = RIGHT; max_cos=cos_right;}
	
 // DEBUG INFO
	if(mode == DEBUG_MODE)
	{
		print(offsetX, 70+offsetY, "Cos_U: %f", cos_up);
		print(offsetX, 90+offsetY, "Cos_D: %f", cos_down);
		print(offsetX,110+offsetY, "Cos_L: %f", cos_left);
		print(offsetX,130+offsetY, "Cos_R: %f", cos_right);
	}
	return  ret;
}
